...
<Alert variant="danger" className="mt-4">
	<AlertTitle>
		Do not use plain `npx shadcn@latest init` command. Use the command below
		instead.
	</AlertTitle>
</Alert>
...
```bash title="Terminal"
npm install tw-animate-css @base-ui-components/react tailwind-merge clsx
```

<Step>Create utils function</Step>

Create `src/lib/utils.ts` (or update if it exists).
...
<Step>Update your CSS file</Step>
...
<Step>Configure TypeScript paths (optional)</Step>

If you want to use the `@/*` import alias, add this to your `tsconfig.json`.
