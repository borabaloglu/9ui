...
export function processComponentPreview(node: any) {
  ...
  return [
    mdx.heading(3, 'Example'),
    mdx.code(demo.source, 'tsx')
  ]
}
 ...
 * Converts automatic/manual installation tabs to structured sections
 */
export function processInstallationTabs(node: any) {
  ...
  const divElements = children.filter((child: any) => 
    child.type === 'mdxJsxFlowElement' && child.name === 'div'
  )
  
  if (divElements.length >= 2) {
    // First div is typically automatic installation
    result.push(mdx.heading(3, 'Automatic Installation'))
    if (divElements[0] && divElements[0].children) {
      result.push(...processInstallationDiv(divElements[0]))
    }
    
    // Second div is typically manual installation  
    result.push(mdx.heading(3, 'Manual Installation'))
...
export function processComponentAnatomy(node: any) {
  ...
  return [
    mdx.heading(3, 'Anatomy'),
    mdx.code(content, 'tsx')
  ]
}
...
export function processTabs(node: any) {
  ...
  for (const tabContent of tabsContents) {
    const valueAttr = tabContent.attributes?.find((attr: any) => attr.name === 'value')
    const tabValue = valueAttr?.value || 'Tab'
    
    result.push(mdx.heading(3, tabValue.charAt(0).toUpperCase() + tabValue.slice(1)))
...
export function processAlert(node: any) {
  const variantAttr = node.attributes?.find((attr: any) => attr.name === 'variant')
...
export function processComponentSource(node: any) {
  ...
  return [
    ...
    mdx.paragraph(`The source code for the ${componentName} component can be found in your components directory after installation.`),
    mdx.code(`// Example usage\nimport { ${componentName} } from "@/components/ui/${componentName.toLowerCase()}"`, 'tsx')
  ]
}
...
export function processThemeToggle(node: any) {
  return [
    mdx.paragraph('Theme toggle component for switching between light and dark modes.')
  ]
}
...
export function processIcons(node: any) {
  return [
    mdx.paragraph('Icon component for displaying various icons.')
  ]
}
